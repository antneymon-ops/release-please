...
export class GitHub {
    ...
    for (const graphCommit of commits) {
      ...
      if (mergePullRequest) {
        if (
          ...
          // Optimization: some pull requests can have thousands of files,
          // the REST API becomes a bottleneck in this situation. We instead
          // paginate through the GraphQL API pull request files API.
          commit.files = await this.getAllPullRequestFiles(
            mergePullRequest.number,
            mergePullRequest.files
          );
        } else {
  ...
  private async getAllPullRequestFiles(
    ...
    const filePaths = files.nodes.map(node => node.path);
    let cursor: string | undefined = files.pageInfo.endCursor;
    let hasNextPage = files.pageInfo.hasNextPage;
    while (hasNextPage) {
      ...
      response.repository.pullRequest.files.nodes.forEach(node =>
