...
export class GitHub {
  ...
  private async mergeCommitsGraphQL(
    ...
    const commitsToBackfill: {commit: Commit; sha: string}[] = [];
    for (const graphCommit of commits) {
      ...
      if (mergePullRequest) {
        ...
        commitsToBackfill.push({commit, sha: graphCommit.sha});
      }
      commitData.push(commit);
    }

    // ⚡ Bolt: Backfill files for commits in parallel to speed up the process.
    // This avoids a sequential loop (N+1 problem) by batching all the API
    // calls together.
    if (commitsToBackfill.length > 0) {
      this.logger.info(
        `Backfilling files for ${commitsToBackfill.length} commits in parallel.`
      );
      const backfillPromises = commitsToBackfill.map(item =>
        this.getCommitFiles(item.sha)
      );
      const fileLists = await Promise.all(backfillPromises);
      fileLists.forEach((files, index) => {
        commitsToBackfill[index].commit.files = files;
      });
    }
    return {
  ...
  /**
   * Paginate through all files in a pull request.
   *
   * @param {number} pullNumber The pull request number
   * @param {string} cursor The cursor to start paginating from
   * @returns {{files: string[], pageInfo: {hasNextPage: boolean, endCursor: string | undefined}}}
   * @throws {GitHubAPIError} on an API error
   */
  private async paginateFilesGraphQL(
    pullNumber: number,
    cursor: string
  ): Promise<{
    files: string[];
    ...
    this.logger.debug(
      `Paginating files for PR #${pullNumber} with cursor ${cursor}`
    );
    const response = await this.graphqlRequest({
      query: `query paginateFiles($owner: String!, $repo: String!, $pullNumber: Int!, $cursor: String) {
        repository(owner: $owner, name: $repo) {
    ...
    return {
      files: files.nodes.map((node: {path: string}) => node.path),
