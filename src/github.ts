...
export class GitHub {
    ...
    for (const graphCommit of commits) {
      ...
      if (mergePullRequest) {
        if (
          ...
          this.logger.info(
            `PR #${mergePullRequest.number} has many files, backfilling with GraphQL`
          );
          // Optimization: use GraphQL to fetch all files since it's more
          // efficient than the REST API fallback.
   ...
   * This method will paginate through all the files in the pull request.
   *
  ...
  async getPullRequestFiles(number: number): Promise<string[]> {
    this.logger.debug(`Fetching file list for PR: ${number}`);
    const files: string[] = [];
    let cursor: string | undefined = undefined;
    while (true) {
      const response = await this.graphqlRequest({
      ...
      for (const f of pullRequestFiles.nodes || []) {
        files.push(f.path);
      }
